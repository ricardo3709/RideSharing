...
Date: 5/2/23
Considers fleet of fossil-fuel vehicles
"""
# %%
import numpy as np
...
for k in range(tData.num_min):
    ...
    for v in vehicles:
        if isinstance(v.request, RideRequest):
            ride_cars += 1
    ride_time.append(ride_cars)
    idle_time.append(n_veh - (ride_cars))
    ...
    for i in range(numRideReq):
        ...
        for j in range(n_veh):
            ...
            if vehicles[j].request is not None or delta_ride < len(start_path[j]) - 1:
    ...
    for i in range(n_assign):
        ...
        if isinstance(req_vec[i], RideRequest):
            ...
            vehicles[veh_idx].estimated_done_riding = k + (len(vehicles[veh_idx].path) - 1) * 10 + min_travel_time
        else:
...
# Plots
ride_time = np.rint(ride_time)
idle_time = np.rint(idle_time)
...
ax7.bar(h_format, idle_time, width=1.0, alpha=0.6, label="Idling", color = "#C53AA6")
ax7.bar(h_format, ride_time, width=1.0, alpha=0.5, label="Riding", color = "#3AA6C5")
...
p3 = ax3.bar(h_format[::time_slot], np.rint(np.sum(np.array(miss_ride_time).reshape(len(miss_ride_time) // time_slot, time_slot), axis=1)), width=2, alpha=0.3, color = "#C8377E", label=("Total missed ride requests: " + str(int(sum(np.rint(np.sum(np.array(miss_ride_time).reshape(len(miss_ride_time) // time_slot, time_slot), axis=1)))))))
