...
Date: 5/2/23
Case 2. Considers EV fleet and shared rides.
...
for it in range(num_iter):
    ...
    for k in range(tData.num_min): #num_min: minutes
        ...
        for j in range(n_veh):
            if len(vehicles[j].path) >= 2 and (k - vehicles[j].last_update) // travel_edge_time == 1:
                vehicles[j].last_update = k
                vehicles[j].path.pop(0)
                vehicles[j].position = vehicles[j].path[0]  # Update the vehicle position to next node
        ...
        for j in range(n_veh):
            ...
            if vehicles[j].request is not None and isinstance(vehicles[j].request, RideRequest) and vehicles[j].estimated_done_riding <= k \
                    and vehicles[j].path[0] == vehicles[j].request.end and len(vehicles[j].path) == 1:
                vehicles[j].request = None  # Vehicle again available
                vehicles[j].capacity = 4
            # If vehicle reaches charging station
            if vehicles[j].request is not None and isinstance(vehicles[j].request, ChargeRequest) \
                    and vehicles[j].path[0] == vehicles[j].request.location and len(vehicles[j].path) == 1:
        ...
        for i in range(numRideReq):
            ride_req = f.create_ride_request(PULoc[i], DOLoc[i], k, g, h_bid[i], np.random.binomial(1, sharing_prob))
            ...
            for j in range(n_veh):
                ...
                if isinstance(vehicles[j].request, ChargeRequest) or \
                ...
                elif isinstance(vehicles[j].request, RideRequest): # Check if the ride can be shared
        ...
        for kreq in range(ride_req_idx):
            for kk in range(n_veh):
                yStar[kk + (n_veh + skip) * kreq] = req_vec[kreq].bid - alpha_w * (
                            len(req_vec[kreq].path) + len(paths[kreq][kk]) - 2) + beta_w * (
                        ev_capacity - vehicles[kk].capacity)
            if n_veh < req_idx:
                skip = req_idx - n_veh
        ...
        for i in range(n_assign):
            ...
            if isinstance(req_vec[i], RideRequest):
                vehicles[veh_idx].capacity -= 1
                vehicles[veh_idx].sharing = req_vec[i].share_ride
                paths[i][veh_idx].pop()
                vehicles[veh_idx].path = paths[i][veh_idx] + req_vec[i].path  # Assign path (pickup + ride)
                vehicles[veh_idx].last_update = k
    ...
    print("num of shared ride: ", num_sharedRide)
...
plt.savefig("ShaSu_PrefMissRide_avg.pdf", bbox_inches='tight')
...
fig3.savefig('ShaSu_SOC_avg.pdf', bbox_inches='tight')
...
fig7.savefig('ShaSu_EV_avg.pdf', bbox_inches='tight')
...
fig4.savefig('ShaSu_power4stations_avg.pdf', bbox_inches='tight')
...
fig5.savefig('ShaSu_incAssign_avg.pdf', bbox_inches='tight')
